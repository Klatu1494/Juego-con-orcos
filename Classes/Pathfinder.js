class Pathfinder {
	...
	encontrarCaminoA(ejercitoOUnidad, coordenadas) {
		if (ejercitoOUnidad.posicion === coordenadas) return;
		this.costoDelCaminoMasCortoEncontrado = Infinity;
		this.caminoMasCortoEncontrado = null;
		if (coordenadas instanceof Coordenadas2D && (ejercitoOUnidad instanceof Unidad || ejercitoOUnidad instanceof Ejercito) && juego.mapaActual) {
			if (juego.mapaActual.hexagonos[coordenadas.y][coordenadas.x].tipo.sePuedeEntrar) {
				for (let hexagonos of juego.mapaActual.hexagonos) {
				...
				this.encontrarCamino(ejercitoOUnidad.posicion, coordenadas, 0, [], []);
			}
